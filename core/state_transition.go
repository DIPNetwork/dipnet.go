...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	if msg.To() == nil {
		...
		addressType = GetAddressType(st.state.GetState(st.to(), HashTypeString("type")))
	...
	if msg.To() == nil {
		...
		switch addressType {
		case "template":
			templateCode := st.state.GetCode(st.to())
			if len(st.data) > 0 {
				templateCode = ByteAndByte(templateCode, st.data)
			}
			coinbase := st.state.GetState(st.to(), HashTypeString("coinbase"))
